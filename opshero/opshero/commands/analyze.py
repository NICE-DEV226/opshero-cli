...
async def _analyze(
    ...
    if offline or not cfg.is_authenticated:
        ...
        async with OpsHeroClient(cfg) as client:
            ...
            project_id = None
            try:
                # 1. Use active project from config if set
                if cfg.active_project_id:
                    project_id = cfg.active_project_id
                    if output == "rich":
                        console.print(
                            f"  [dim]Project context[/dim]  "
                            f"[{_B}]{escape(cfg.active_project_name or cfg.active_project_id)}[/{_B}]"
                        )
                else:
                    # 2. Auto-detect from git repo
